...
static bool is_momentary_layer = false;

bool process_key_trap(keyrecord_t*);
bool trap_process_key(uint16_t, keyrecord_t*);
bool process_alpha_on_caps_off(keyrecord_t*);
bool process_caps_on(keyrecord_t*);
...
layer_state_t layer_state_set_user(layer_state_t state) {
    switch (get_highest_layer(state)) {
        case ALPHA_LAYER:
            is_momentary_layer = false;
            break;
        case GAME_LAYER ... MOUS_LAYER:
            is_momentary_layer = false;
            caps_word_off();
            break;
        case MO_ALP_LYR ... MO_MSE_LYR:
...
bool process_record_user(uint16_t keycode, keyrecord_t* record) {
    switch (keycode) {
        ...
        case ALPHA_ON_CAPS_OFF:
            return process_alpha_on_caps_off(record);
        case CAPS_ON:
            return process_caps_on(record);
        case KEY_TRAP:
            return process_key_trap(record);

        default: return true;
    }
}
bool process_key_trap(keyrecord_t* record) {
...
bool process_alpha_on_caps_off(keyrecord_t* record) {
    if (record->event.pressed) {
        caps_word_off();
        if (!is_momentary_layer) layer_move(ALPHA_LAYER);
    }
    return false;
}
bool process_caps_on(keyrecord_t* record) {
    if (record->event.pressed)
        caps_word_on();
    return false;
}
...
bool trap_process_key(uint16_t keycode, keyrecord_t* record) {
    ...
    if (trap.state == HOLD && trap.size < MAX_HOLD) {
        if (trap.free_key == keycode) trap.free_key = KC_NO;
